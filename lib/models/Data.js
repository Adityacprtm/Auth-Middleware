(function () {
    ...
    module.exports = (app) => {
        ...
        Data.request = function (payload) {
            var deviceId, secretKey, expiration, token, data
            deviceId = payload.id
            secretKey = payload.secret
            expiration = {
                expiresIn: '3h'
            }
            token = jwt.sign(payload, secretKey, expiration)
        ...
        Data.validity = function (token, callback) {
            ...
            return Data.find('devices', deviceId, (err, data) => {
                if (err != null) {
                    callback(err)
                    return
                } else {
